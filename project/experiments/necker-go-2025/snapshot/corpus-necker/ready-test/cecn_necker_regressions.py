...
def weights(server, name):
    ...
    result = {}
    for layer, body in re.findall('<Lay ([^>]+)>(.*?)</Lay>', path.read_text(), re.S):
        for destination, unit in re.findall('<UgUn (\\d+) [^>]*>(.*?)</UgUn>', body, re.S):
            for sender, group in re.findall('<Cg \\d+ Fm:([^>]+)>(.*?)</Cg>', unit, re.S):
                for source, value in re.findall('^(\\d+) ([\\d.eE+-]+)$', group, re.M):
                    key = (layer, int(destination), sender, int(source))
                    require(key not in result, 'duplicate connection')
                    result[key] = float(value)
    require(result, 'native weights missing')
    return result

def collect(case, server, name, cycles, adapt):
    data = server.call('GetData', table='CycleOutputData')
    columns = {column['name']: column['values'] for column in data['columns']}
    acts = list(map(flat, columns['act']))
    net = list(map(flat, columns['net']))
    act_eq = list(map(flat, columns['act_eq']))
    harmony = columns['harmony']
    ...
    case.equal(name + '.author_harmony_formula', all((math.isclose(sum((n * a for n, a in zip(nr, ar))) / 16, h, rel_tol=1e-06, abs_tol=1e-07) for nr, ar, h in zip(net, act_eq, harmony))), True)
    differences = [sum(row[:8]) / 8 - sum(row[8:]) / 8 for row in acts]
    states = [1 if delta > 0.5 else -1 if delta < -0.5 else 0 for delta in differences]
    decided = [state for state in states if state]
    switches = sum((a != b for a, b in zip(decided, decided[1:])))
    for channel in ['gc_kna_f', 'gc_kna_m', 'gc_kna_s']:
        values = flat(columns[channel])
        case.equal(name + '.' + channel + '_finite_nonnegative', all((math.isfinite(value) and value >= 0 for value in values)), True)
...
def coherent(case, result):
    ...
    winner = 0 if sum(last[:8]) > sum(last[8:]) else 1
    active = last[winner * 8:(winner + 1) * 8]
    inactive = last[(1 - winner) * 8:(2 - winner) * 8]
    case.equal(result['name'] + '.all_winning_vertices_active', min(active) > 0.5, True)
    case.equal(result['name'] + '.other_interpretation_inactive', max(inactive) < 0.01, True)
    case.equal(result['name'] + '.exactly_eight_active_vertices', sum((value > 0.5 for value in last)), 8)
    case.equal(result['name'] + '.settling_increases_harmony', result['harmony'][-1] > result['harmony'][0], True)
    return winner

def run(case):
    setup_source = case.args.fixture
    provenance = json.loads(case.args.provenance.read_text())
    ...
    shutil.copy2(setup_source, case.fixture)
    with Server(case) as server:
        setup(server)
        a = trial(case, server, 'default_seed1', 0.01, False, 25, 1)
        b = trial(case, server, 'default_seed3', 0.01, False, 25, 3)
        ...
        for name, variance in [('noise_high', 0.1), ('noise_low', 0.001)]:
            coherent(case, trial(case, server, name, variance, False, 25, 1))
        control = trial(case, server, 'long_no_adaptation', 0.01, False, 250, 1)
        adapt = trial(case, server, 'long_adaptation', 0.01, True, 250, 1)
        ...
        saved = case.directory / 'necker_cube-saved.proj'
        server.console('.projects[0].SaveCopy(' + json.dumps(str(saved)) + '); cout << "NECKER_SAVED" << endl;', 'NECKER_SAVED')
    reopened = Case(case.args, 'reopened')
    reopened.fixture = saved
    with Server(reopened) as server:
        setup(server)
        repeat = trial(reopened, server, 'adaptation_reopened', 0.01, True, 250, 1)
    ...
    for item in [case, reopened]:
        warnings = [line for line in (item.directory / 'emergent.log').read_text(errors='replace').splitlines() if re.search('^Warning:|^Error:', line)]
...
def main():
    ...
    parser.add_argument('--fixture', type=Path, default=REPO / 'demo/LegacyModels/cecn/chapter_3/necker_cube.proj')
    parser.add_argument('--provenance', type=Path, default=REPO / 'demo/LegacyModels/cecn/chapter_3/necker-provenance.json')
    parser.add_argument('--output', type=Path, required=True)
    parser.add_argument('--timeout', type=float, default=90)
    args = parser.parse_args()
    for name in ['binary', 'fixture', 'provenance', 'output']:
    ...
    case = Case(args, 'necker_cube')
    report = {'status': 'PASS', 'checks': case.checks, 'installed_sha256': hashes, 'scope': 'Authored maintained8.5 FFFB/net-input-noise/KNa model; no old kWTA/AdEx equivalence claim. Added observers only in test copies.'}
    try:
        run(case)
    except Exception as error:
        report.update(status='FAIL', error=str(error), traceback=traceback.format_exc())
    for path, digest in hashes.items():
        require(hashlib.sha256(Path(path).read_bytes()).hexdigest() == digest, 'installed library changed during test')
