...
public:
  enum Numerics {
    LEGACY_CPP,                     // historical Emergent activation and conductance arithmetic
    GO_LEABRA_2025                  // pinned Go Leabra 79e931d: FastExp activation, signed Ge, and E+L+I+K current order
  };
  Numerics      numerics;           // #DEF_LEGACY_CPP #EXPERT select the numerical conventions required by the model; historical projects retain LEGACY_CPP
  ...
  INLINE float  NoisyXX1(float x) {
    if(x < 0.0f) {        // sigmoidal for < 0
      if(numerics == GO_LEABRA_2025) {
        const float exponent = -(x * sig_gain_nvar);
        if(exponent > 50.0f) return 0.0f;
        return sig_mult_eff / (1.0f + LeabraGoFastExp(exponent));
      }
    ...
    if(x < interp_range) {
      ...
      if(x < 0.0f) {        // sigmoidal for < 0
        if(numerics == GO_LEABRA_2025) {
          const float exponent = -(x * sig_gain_nvar);
          if(exponent > 50.0f) return 0.0f;
          return sig_mult_eff_arg / (1.0f + LeabraGoFastExp(exponent));
        }
...
public:
  ...
  float         ru_lrn_m;            // #DEF_0.1;0 #MIN_0 #MAX_1 #AKA_m_in_s how much of the medium term average activation to mix in with the short (plus phase) to compute the ru_avg_s_lrn variable that is used for the receiving unit's short-term average in learning -- for DELTA_FF_FB it should be 0 -- not needed -- for XCAL_CHL this is important to ensure that when unit turns off in plus phase (short time scale), enough medium-phase trace remains so that learning signal doesn't just go all the way to 0, at which point no learning would take place -- typically need faster time constant for updating s such that this trace of the m signal is lost -- can set ss_tau=7 and set this to 0 but learning is generally somewhat worse
  float         su_lrn_m;            // #DEF_0.5;0.1;0 #MIN_0 #MAX_1 how much of the medium term average activation to mix in with the short (plus phase) to compute the su_avg_s_lrn variable that is used for the sending unit's short-term average in learning -- for DELTA_FF_FB delta-rule based learning, this is typically .5 (half-and-half) but for XCAL_CHL it is typically the same as ru_lrn_m (.1)
...
public:
  ...
  ModMode       mod;            // #DEF_LAY_ERR_MOD whether and how to modulate amount of BCM hebbian learning as function of other variables
  float         lay_mod_min;    // #DEF_0.01 #CONDSHOW_ON_mod:LAY_ERR_MOD minimum mod_avg_l_lrn modulation value for LAY_ERR_MOD -- ensures a minimum amount of self-organizing learning even for network / layers that have a very small level of error signal
  float         net_mod_min;    // #DEF_0.5 #CONDSHOW_ON_mod:NET_ERR_MOD minimum mod_avg_l_lrn modulation value for NET_ERR_MOD -- ensures a minimum amount of self-organizing learning even when error is low
  float         net_err_start;  // #DEF_0.5 #CONDSHOW_ON_mod:NET_ERR_MOD for NET_ERR_MOD, do not start decreasing the amount of hebbian learning until the network avg_cos_err has gone above this value (higher is better, 1 = perfect) -- beneficial to wait until learning as progressed well before starting to back off on hebbian -- modulation effect goes from 1 to mod_min within the range of avg_cos_err between err_start and err_end
  float         net_err_end;    // #DEF_1 #CONDSHOW_ON_mod:NET_ERR_MOD for NET_ERR_MOD, upper level of network avg_cos_err (higher is better, 1 = perfect) where the modulatory factor will reach mod_min (and stay there) -- can set less than 1 if model doesn't fully converge to get full range of modulation
