#!/usr/bin/env python3
import hashlib,json,math,os,shutil,sys,traceback,gc
...
def paged_data(server, count):
 ...
 for start in range(0,count,100):
  page=server.call('GetData',table='CycleOutputData',row_from=start,rows=min(100,count-start))
  if values is None:values=page
  else:
   for target,column in zip(values['columns'],page['columns']):
    require(target['name']==column['name'],'paged table schema changed');target['values'].extend(column['values'])
 return values

root=Path('/home/b/cemer');base=root/'artifacts/go-models';args=SimpleNamespace(output=base/'necker-native-validation2',binary=root/'project/tools/run-emergent',timeout=360)
...
try:
 for name,noise,adapt,quarter in [('zero-noise',0,False,25),('zero-noise-adaptation',0,True,250),('authored-default',.01,False,25),('authored-adaptation',.01,True,250)]:
  c=Case(args,name);c.fixture=c.directory/'necker_cube.proj';shutil.copy2(base/'necker-native-candidate1/necker_cube.proj',c.fixture)
  ...
  with Server(c) as s:
   ...
   s.run('LeabraEpoch');values=paged_data(s,100*cycles);(c.directory/'cycles.json').write_text(json.dumps(values)+'\n');cols={x['name']:x['values'] for x in values['columns']}
   c.equal('all100_trials_ran',len(cols['cycle']),100*cycles)
   c.equal('correct_cycle_counters',cols['cycle'],[i%cycles+1 for i in range(100*cycles)])
   c.equal('correct_trial_counters',cols['run_no'],[i//cycles+1 for i in range(100*cycles)])
   for go,cpp in [('Noise','noise'),('Act','act'),('Ge','net'),('Gi','gc_i'),('Vm','v_m_eq'),('KFast','gc_kna_f'),('KMed','gc_kna_m'),('KSlow','gc_kna_s')]:
    ...
    item['maximum_absolute_differences'][go]={'maximum':maximum,'at':at}
   ...
   c.equal('noise_replay_exact_float32',item['maximum_absolute_differences']['Noise']['maximum']<1e-8,True)
   c.equal('activation_within_declared_tolerance',item['maximum_absolute_differences']['Act']['maximum']<(0.01 if noise else 0.00001),True)
