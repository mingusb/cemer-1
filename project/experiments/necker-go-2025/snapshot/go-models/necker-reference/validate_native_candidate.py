...
root=Path('/home/b/cemer');base=root/'artifacts/go-models';args=SimpleNamespace(output=base/'necker-native-validation1',binary=root/'project/tools/run-emergent',timeout=360)
...
try:
 for name,noise,adapt,quarter in [('zero-noise',0,False,25),('zero-noise-adaptation',0,True,250),('authored-default',.01,False,25),('authored-adaptation',.01,True,250)]:
  ...
  with Server(c) as s:
   ...
   s.run('LeabraEpoch');values=s.call('GetData',table='CycleOutputData');(c.directory/'cycles.json').write_text(json.dumps(values)+'\n');cols={x['name']:x['values'] for x in values['columns']}
