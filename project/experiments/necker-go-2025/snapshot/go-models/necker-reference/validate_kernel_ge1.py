...
root=Path('/home/b/cemer');base=root/'artifacts/go-models';args=SimpleNamespace(output=base/'necker-kernel-ge-validation1',binary=root/'project/tools/run-emergent',timeout=360)
...
try:
 for name,noise,adapt,quarter in [('authored-high-noise',.1,False,25),('authored-low-noise',.001,False,25)]:
  c=Case(args,name);c.fixture=c.directory/'necker_cube.proj';shutil.copy2(base/'necker-native-candidate5/necker_cube.proj',c.fixture)
   ...
   for go,cpp in [('Noise','noise'),('Act','act'),('Ge','net'),('Gi','gc_i'),('Vm','v_m_eq'),('KFast','gc_kna_f'),('KMed','gc_kna_m'),('KSlow','gc_kna_s')]:
    ...
    for row,actual in zip(rows,cols[cpp]):
     ...
     for ui,(a,b) in enumerate(zip(row[go],vals)):
      delta=abs(struct.unpack("f",struct.pack("f",a))[0]-struct.unpack("f",struct.pack("f",b))[0])
