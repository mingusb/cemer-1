    ...
    if(itd->parents.size >= 1) {
      if(new_type->InheritsFrom(itd->GetParent())) {
        ...
        TestError(true, "Changetype", "Cannot change to new type:",new_type->name,
                  ...
                  "(or vice-versa)",itm->GetPathNames(this));
        return false;
      }
    }
    else {
...
int taList_impl::ReplaceValStr(const String& srch, const String& repl, const String& mbr_filt,
  ...
  int rval = inherited::ReplaceValStr(srch, repl, mbr_filt, par, par_typ, memb_def, sc, replace_deep);
  if (replace_deep) {
    for(int i=0; i<size; i++) {
      taBase* itm = (taBase*)el[i];
      if(itm && itm->GetOwner() == this) { // only owned is key for preventing recursion
        rval += itm->ReplaceValStr(srch, repl, mbr_filt, this, GetTypeDef(), memb_def, sc, replace_deep);
      }
    }
  }
  if(rval > 0)
...
taBase* taList_impl::Dump_Load_Path_parent(const String& el_path, TypeDef* ld_el_typ) {
  ...
  if(nw_el) {
    ...
    if((nw_el->GetTypeDef() != ld_el_typ) &&
      ...
      if(!nw_el) {
        taMisc::Warning("*** Dump_Load_Path_parent: Could not make new token of type:",
                        ...
                        el_path,"in parent list:",DisplayPath());
        return NULL;
      }
    ...
    if(!nw_el) {
      taMisc::Warning("*** New: Could not make a token of:",ld_el_typ->name,"in:",DisplayPath());
      return NULL;
    }
    if(has_nm) {
      nw_el->SetName(elnm);
...
String taList_impl::GetPath_impl(taBase* ta, taBase* par_stop) const {
...
String taList_impl::GetPathNames_impl(taBase* ta, taBase* par_stop) const {
...
void taList_impl::SearchIn_impl(const String_Array& srch, taBase_PtrList& items,
  ...
  taOBase::SearchIn_impl(srch, items, owners, text_only, contains, case_sensitive, obj_name, obj_type,
  ...
  for(int i=0; i<size; i++) {
    ...
    if(itm->SearchTestItem_impl(srch, text_only, contains, case_sensitive, obj_name, obj_type,
                                obj_desc, obj_val, mbr_name, type_desc)) {
      items.Link(itm);
    }
    itm->SearchIn_impl(srch, items, owners, text_only, contains, case_sensitive, obj_name, obj_type,
                       obj_desc, obj_val, mbr_name, type_desc);
  }
...
int taList_impl::UpdatePointers_NewParType(TypeDef* par_typ, taBase* new_par) {
  ...
  for(int i=size-1; i>=0; i--) {
    ...
    if(itm->GetOwner() == this) { // for guys we own (not links; prevents loops)
      ...
      String old_path = itm->GetPath(old_own);
      MemberDef* md;
      taBase* nitm = new_par->FindFromPath(old_path, md);
      if(nitm) {
        ReplaceLinkIdx_(i, nitm);
