...
iColorScaleBar::iColorScaleBar(bool hor_, SpanMode sm_, bool adj, bool ed, QWidget* parent)
:inherited(parent)
{
  sm = sm_;
  Init(hor_, adj, ed);
...
iColorScaleBar::~iColorScaleBar(){
  padlist.Reset();
}

void iColorScaleBar::UpdatePads(){
  for (int i = 0; i < padlist.size; ++i) {
    ((iColorPad*) padlist.FastEl(i))->Reset();
  }
}

void iColorScaleBar::Init(bool hor_, bool adj, bool ed){
...
void iColorScaleBar::Adjust(){
  emit_scaleValueChanged();
}

void iColorScaleBar::editor_accept() {
...
void iColorScaleBar::emit_scaleValueChanged() {
  emit scaleValueChanged();
}

/* implicit
void iColorScaleBar::editor_reject(ivFieldEditor*) {
  UpdateScaleValues();
  UpdatePads();
  Adjust();
} */
...
const iColor iColorScaleBar::GetColor(int idx, bool* ok){
  int i = idx;
  if(i < 0) i = bar->scale->colors.size + i;
  return(bar->scale->GetColor(i, ok));
}

const iColor iColorScaleBar::GetContrastColor(int idx, bool* ok){
  int i = idx;
  if(i < 0) i = bar->scale->colors.size + i;
  return(bar->scale->GetContrastColor(i, ok));
}

float iColorScaleBar::GetVal(int idx) {
  return (((idx * (max()-min()))/(bar->blocks()-1)) + min());
}

bool iColorScaleBar::GetScaleValues(){
  bool ok = true;
  if (editflag) {
...
void iColorScaleBar::SetMinMax(float mn,float mx){
