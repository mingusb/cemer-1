...
public:
  ...
  virtual void          CheckUpdateDataPanelSet(iPanelSet* pan) { (void)pan; } // dynamically updates a data panel set; currently only called when set gets a USER_DATA_UPDATED notify
  virtual taiSigLink*  GetSigLink(void* data_, TypeDef* el_typ) { (void)data_; (void)el_typ; return NULL;}
    // get an existing, or create new if needed
  virtual const iColor  GetEditColorInherit(taiSigLink* dl, bool& ok) const { (void)dl;ok = false; return def_color;} // #IGNORE background color for edit dialog, include inherited colors from parents
...
protected:
  ...
  virtual void          CreateDataPanel_impl(taiSigLink* dl_) { (void)dl_; } // create one or more dp's
