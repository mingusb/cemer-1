  ...
  for (int i = 0; i < ms.size; ++i) {
    ...
    if (!md->IsEditorHidden() && !md->HasExpert()) {
      if (Base() && Base()->GetMemberOwner() && md->name == "name") {
        continue;               // don't add names of members..
        // note: was previously changing type info
      }
      if (md->IsGuiReadOnly()) {
...
//      return true;
  ...
  if(ctrl_pressed && ((e->key() == Qt::Key_Return) || (e->key() == Qt::Key_Enter))) {
    if(modal)
      ...
      Apply();
    if(host_type != HT_DIALOG && mvw) {
      taMisc::DebugInfo("taiEditorOfClass returning focus to tree view");
