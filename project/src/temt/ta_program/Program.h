...
public:
  ...
  static ProgLib*       prog_lib; // #TREE_HIDDEN #NO_SAVE #NO_FIND #HIDDEN library of available programs

  Program_Group*        prog_gp;
  // #HIDDEN #READ_ONLY #NO_SAVE #NO_SET_POINTER #HIDDEN #CAT_IGNORE our owning program group
  ...
  ScriptSource scriptSource() override {return ScriptString;}
  const String scriptString() override;
  ...
  // (re)load the program from the program library -- lookup by given name -- used by web interface to auto-update a new program
  ...
  // #MENU #MENU_CONTEXT #MENU_SEP_BEFORE #NO_BUSY #CAT_Code view the associated code in the program editor frame
...
public: // XxxGui versions provide feedback to the user
  ...
  virtual bool          AddCtrlFunsToControlPanel
    (ControlPanel* ctrl_panel, const String& extra_label = "", const String& sub_gp_nm = "");
  // #MENU #MENU_SEP_BEFORE #MENU_ON_ControlPanel #NULL_OK_0 #NULL_TEXT_0_NewCtrlPanel #PREFER_ITEM #ITEM_FILTER_0_ControlPanelNoParamSetItemFilter #CAT_Display add the program control functions (Init, Run, Step, Stop) to a project control_panel - a panel that is a user chosen collection of members and methods from one or more objects (if ctrl_panel is NULL, a new one is created in .ctrl_panels). returns false if method was already selected. extra_label is prepended to item names, and if sub_gp_nm is specified, items will be put in this sub-group (new one will be made if it does not yet exist)
  ...
  virtual bool          BrowserEditMe_ProgItem(taOBase* itm);
  // #IGNORE perform BrowserEditMe function for program sub-item (prog el, etc)
...
protected:
  ...
  void                  HasCallCycle(Program* program, Program_List* call_stack);  // check for Program A calling B calling A or a more indirect call cycle - either is bad!
