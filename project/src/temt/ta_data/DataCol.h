...
public:
  ...
  int           FindValPartialCi(const Variant& val, int st_row = 0) const;
  // #CAT_Access case insensitive version find row number for given value if it is contained within any element of column of scalar type (use for Programs), starting at given starting row number.  if st_row < 0 then the search proceeds backwards from that many rows from end (-1 = end)
  ...
  // #CAT_ObjectMgmt change the type of the data in this col, without loss of data
  void          ChangeColType_gui(ValType new_type);
  // #CAT_ObjectMgmt #BUTTON #MENU #DYN1 #LABEL_ChangeColType change the type of the data in this col, without loss of data
