...
double taMath_double::beta_den(double x, double a, double b) {
  if(x < 0.0 || x > 1.0) {
    return 0 ;
  }
  else {
...
(double_Matrix* vec, const double_Matrix* oth, double bin_size,
 double min_val, double max_val, double_Matrix* bin_vec) {
...
void taMath_double::vec_histogram_bins(double_Matrix* vec, const double_Matrix* oth,
                                      double bin_size, double min_val, double max_val) {
  if(!vec_check_type(vec) || !vec_check_type(oth)) return;
  if(oth->size == 0) return;
  vec->SetGeom(1,0);
  vec->Reset();
...
bool taMath_double::fft_complex(complex_Matrix* mat) {
  ...
  bool rval = false;
  static gsl_fft_complex_wavetable* gwt = NULL;
  static gsl_fft_complex_workspace* work = NULL;
...
bool taMath_double::fft2_complex(complex_Matrix* mat) {
  ...
  // make scratch -- declare all, so we can jump to exit
  bool rval = false;
  static gsl_fft_complex_wavetable* gwt_rows = NULL;
  static gsl_fft_complex_workspace* work_rows = NULL;
  static gsl_fft_complex_wavetable* gwt_cols = NULL;
  static gsl_fft_complex_workspace* work_cols = NULL;
...
bool taMath_double::mat_frame_prod(double_Matrix* out_mat, const double_Matrix* in_mat) {
  ...
  int fsi = in_mat->FrameStartIdx(0);
  for(int i=0;i<frs;i++) {
    double prod = in_mat->FastEl_Flat(fsi + i);
    for(int j=1;j<frn;j++) {
      fsi = in_mat->FrameStartIdx(j);
...
bool taMath_double::mat_frame_var(double_Matrix* out_mat, const double_Matrix* in_mat, bool use_est) {
  ...
  for(int i=0;i<frs;i++) {
    ...
    for(int j=0;j<frn;j++) {
      int fsi = in_mat->FrameStartIdx(j);
      double dm = (in_mat->FastEl_Flat(fsi + i) - mean);
      var += dm * dm;
    }
