...
//   static jmp_buf     begin;
  ...
  static TimeUsedHR     proc_events_timer;   // timer used in deciding when to process gui events
...
#define CSS_REGISTER 
  ...
  { tmp_str = GetStr(); return (const char*)tmp_str; }
  // need to hold onto str otherwise char* dissappears

  // these misc "nuisance" equivalent type converters and are typically not overwritten
  // except for exact c_ptr types
  virtual operator char*() const                { CvtErr("(char*)"); return NULL; }
