...
iDialogObjChooser* iDialogObjChooser::createInstance(taBase* parob, const char* captn, bool selonly, QWidget* par_window_) {
  if (par_window_ == NULL)
    par_window_ = taiMisc::main_window;
  return new iDialogObjChooser(parob, captn, selonly, par_window_);
}

iDialogObjChooser* iDialogObjChooser::createInstance(TypeDef* tpdf, const char* captn, taBase* scope_ref_, QWidget* par_window_) {
  if (par_window_ == NULL)
    par_window_ = taiMisc::main_window;
...
iDialogObjChooser::iDialogObjChooser(taBase* parob, const char* captn, bool selonly, QWidget* par_window_)
{
...
void iDialogObjChooser::setSel_obj(const taBase* value) {
...
void iDialogObjChooser::Clear() {
  browser->clear();
  items.Reset();
  editor->clear();
}

void iDialogObjChooser::Load() {
  if(lst_par_obj != NULL) {
    if(!select_only) {
      AddItem("..");
...
void iDialogObjChooser::AddObjects(taBase* obj) {
...
void iDialogObjChooser::AddTokens(TypeDef* td) {
  int i;
...
void iDialogObjChooser::AddItem(const char* itm, const void* data_) {
  items.Add(itm);
...
void iDialogObjChooser::UpdateFmSelStr() {
  msel_obj = NULL;
  MemberDef* md;
  if(lst_par_obj != NULL) {
    if(msel_str == "root")
...
void iDialogObjChooser::browser_currentItemChanged(QListWidgetItem* itm, QListWidgetItem*) {
  GetPathStr();
  if (itm == NULL)
    msel_obj = NULL;
  else
...
void iDialogObjChooser::DescendBrowser() {
  int i = browser->currentRow();
  if (i == -1) {
...
void iDialogObjChooser::reject() {
  msel_obj = NULL;
  iDialog::reject();
}

void iDialogObjChooser::AcceptEditor() {
  AcceptEditor_impl(editor);
}

void iDialogObjChooser::AcceptEditor_impl(QLineEdit* e) {
