...
taiEditorOfWizard::taiEditorOfWizard(taWizard* base_, TypeDef* typ_,
...
taiEditorOfWizard::~taiEditorOfWizard() {
}

/*void taiEditorOfWizard::Constr(const char* prompt, const char* win_title) {
  inherited::Constr(prompt, win_title);
}*/

void taiEditorOfWizard::Constr_Box() {
  tabs = new QTabWidget(widget());
  vblDialog->addWidget(tabs, 1);
}

void taiEditorOfWizard::Constr_RegNotifies() {
...
void taiEditorOfWizard::Constr_Buttons() {
...
/*void taiEditorOfWizard::btnPrint_clicked() {
  QPrinter pr;
...
/*void taiEditorOfWizard::GetImage() {
}
...
void taiEditorOfWizard::Ok_impl() { //note: only used for Dialogs
  inherited::Ok_impl();
  //  if (modified) {
    GetValue();
    Unchanged();
