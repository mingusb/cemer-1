...
taiWidgetDelegate::taiWidgetDelegate(taiEditorOfClass* edh_)
{
  edh = edh_;
  m_dat_row = -1;
...
QWidget* taiWidgetDelegate::createEditor(QWidget* parent,
...
bool taiWidgetDelegate::eventFilter(QObject *object, QEvent *event)
{
...
void taiWidgetDelegate::GetImage() const {
  if (!dat) return;
  edh->Updating(true);
...
void taiWidgetDelegate::GetValue() const {
  if (!dat) return;
...
void taiWidgetDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option,
...
void taiWidgetDelegate::this_closeEditor(QWidget* /*editor*/,
